(function() {
	 ...
	 */
	function pull() {
		$('.board').append( //On ajoute au plateau le tetrimino
	...
	if (myTetrisGhost) {
		$(myTetrisGhost).on('ghostCreated', function(e, ghost) {
			...
			$('.board').append(this.ghostTetrimino);
		});
	}
	...
	if (myTetrisScore) {
		$(myTetrisScore).on('scoring', function() {
			...
			$('.score > span').html(myTetrisScore.points);
			$('.score > small > span').html(myTetrisScore.lines);
		});
	}
	...
	if (myTetrisNext) {
		$(myTetrisNext).on('nextUpdated', function(e, nextTetriminos) {
			$('.score > .nexts').children().remove();

			for (let next of nextTetriminos) {
				$('.score > .nexts').append(next);
			}
		});
	}
