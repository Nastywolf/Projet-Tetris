function Tetris(options) {
	...
	if (options.largeur === undefined) options.largeur = 10;
	if (options.hauteur === undefined) options.hauteur = 20;
	if (options.sizeOfBloc === undefined) options.sizeOfBloc = 25;
		...
		for (var y = 0; y < figure.matrice.length; y++) {
			for (var x = 0; x < figure.matrice[y].length; x++) {
				if (figure.matrice[y][x] == 1) {
					...
					myBloc.css('background-color', tetrimino.data('source').couleur);
